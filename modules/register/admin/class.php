...
$sql  = 'SELECT * FROM `nv4_vi_register_classes`';
if (isset($_GET['id'])){
    $sqlEdit  = 'SELECT * FROM `nv4_vi_register_classes` where id='.$_GET['id'];
    $action.='&id='.$_GET['id'];
...
if (isset($_POST['submit'])) {
    ...
    $grade        = $_POST['grade'] ?? '';
    if ($_GET['id'] ?? 0) {
        $sql = "UPDATE `nv4_vi_register_classes` SET `name`= :name,`teacher_name`= :teacher_name, grade = :grade WHERE id=".$_GET['id'];
    } else {
        $sql = 'INSERT INTO `nv4_vi_register_classes`(`name`, grade, `teacher_name`) VALUES (:name, :grade, :teacher_name)';
    }

    $sth = $db->prepare($sql);
    $sth->bindParam(':name', $name, PDO::PARAM_STR);
    $sth->bindParam(':teacher_name', $teacher_name, PDO::PARAM_STR);
...
if (isset($_GET['id_delete'])){
    $id = $_GET['id_delete'];
    $sqlDelete = 'DELETE FROM nv4_vi_register_classes WHERE id='.$id;
